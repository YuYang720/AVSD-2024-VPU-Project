...
set cpu_clk_period 1.2
set wdt_clk_period 50.1

create_clock -name cpu_clk -period $cpu_clk_period [get_ports cpu_clk]
create_clock -name wdt_clk -period $wdt_clk_period [get_ports wdt_clk]
set_clock_groups -asynchronous -group {cpu_clk} -group {wdt_clk}
...
set_ideal_network           [get_pins ipad_wdt_clk/C]
...
# wdt_clk
set input_max_rom_clk   [expr {double(round(1000*$wdt_clk_period * 0.6))/1000}]
set input_min_rom_clk   [expr {double(round(1000*$wdt_clk_period * 0.0))/1000}]
set output_max_rom_clk  [expr {double(round(1000*$wdt_clk_period * 0.1))/1000}]
set output_min_rom_clk  [expr {double(round(1000*$wdt_clk_period * 0.0))/1000}]

set_input_delay  -max $input_max_rom_clk  -clock wdt_clk [remove_from_collection [all_inputs] [get_ports {wdt_clk}]]
set_input_delay  -min $input_min_rom_clk  -clock wdt_clk [remove_from_collection [all_inputs] [get_ports {wdt_clk}]]
set_output_delay -max $output_max_rom_clk -clock wdt_clk [all_outputs]
set_output_delay -min $output_min_rom_clk -clock wdt_clk [all_outputs]
...
#set_driving_cell -library N16ADFP_StdCellss0p72vm40c -lib_cell BUFFD4BWP16P90LVT -pin {Z} [get_ports {cpu_clk wdt_clk}]
#set_driving_cell -library N16ADFP_StdCellss0p72vm40c -lib_cell DFQD1BWP16P90LVT  -pin {Q} [remove_from_collection [all_inputs] [get_ports {cpu_clk wdt_clk}]]
set_driving_cell -library N16ADFP_StdIOss0p72v1p62vm40c -lib_cell PDCDG_H -pin {C} [all_inputs]
set_driving_cell -library N16ADFP_StdIOss0p72v1p62vm40c -lib_cell PDCDG_V -pin {C} [all_inputs]
